...
import { getGPT3ParseExtractInfo, getGPT3CustomPromptCompletion, getGPT3DiarizationPromptCompletion, getGPT3Summarize, getKeyPhrases, getTokenOrRefresh } from './token_util.js';
...
export default class BatchPage extends Component {
      ...
      fetch('/upload', {
        ...
        .then(data => {
          ...
          const getProfileInfos = async () => {
            // var PersonObj = await this.gptPersonInfo(result);
            ...
            // });
            // console.log(this.modifiedConversation);
            ...
            this.setState({ modifiedConversation1: result })
  ...
  async gptPersonInfo(inputText) {
    ...
    var customPromptText = "In the given call center conversation, categorize Speaker 1 and Speaker 2 into 'Agent' or 'Customer' after analysing their behavior. Store the result in a dictionary { key:value } where key-value pair is the speaker and his/her category respectively. Make sure to use double quotes for keys and string values and only return { key:value }";
    // var customPromptText = `Your  input is a speaker based transcription of a conversation between two or more people in hindi or any other langunges. In the transcription, the speakers will only be known as SPEAKER 1, SPEAKER 2, etc. Your goal it to identify the names of the speakers if possible.Return your results in a JSON format { "SPEAKER 1" : "<speaker 1 name>", "SPEAKER 2" : "<speaker 2 name>",   "SPEAKER 3" : "<speaker 3 name>"  }`
    ...
    const gptObj = await getGPT3DiarizationPromptCompletion(inputText, customPromptText);
